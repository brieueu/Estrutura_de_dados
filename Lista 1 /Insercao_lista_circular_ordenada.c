...
bool inserirNaOrdem(ITEM item, LISTA *l){
    ...
    //procura a posicao correta de inser�ao, ordenando por chave
    while (p != l->cabeca && p->item.chave < item.chave)
    {
        ant = p;
        p = p -> prox;
    }

    if (p != l->cabeca && p->item.chave == item.chave){
        return false;
    }

    //cria o novo NO e insere entre ant e p
    NO* novo = criarNo(item, p);
    ant->prox = novo;
    l -> tamanho++;
    return true;
}
